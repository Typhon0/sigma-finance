	...
	for _, ticker := range response {
		for _, priceData := range ticker.PriceData {
			...
			candles = append(candles, model.Candle{
				...
				AssetType: req.AssetType,
				Interval:  req.Interval,
				Open:      model.FromFloat(priceData.Open),
				High:      model.FromFloat(priceData.High),
				Low:       model.FromFloat(priceData.Low),
				Close:     model.FromFloat(priceData.Close),
...
func (t *TiingoProvider) getForexCandles(ctx context.Context, symbol string, req CandleRequest) (*CandleResponse, error) {
	...
	for _, item := range response {
		...
		candles = append(candles, model.Candle{
			...
			Low:       model.FromFloat(item.Low),
			Close:     model.FromFloat(item.Close),
			Volume:    0, // Forex typically doesn't have volume
			Timestamp: timestamp,
			Source:    t.ID(),
		})
	}
