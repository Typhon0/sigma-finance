...
func (f *FinnhubProvider) GetCandles(ctx context.Context, req CandleRequest) (*CandleResponse, error) {
	...
	for i, timestamp := range response.Time {
		...
		candle := model.Candle{
			...
			Open:      decimal.NewFromFloat(response.Open[i]),
			High:      decimal.NewFromFloat(response.High[i]),
			Low:       decimal.NewFromFloat(response.Low[i]),
			Close:     decimal.NewFromFloat(response.Close[i]),
			Volume:    decimal.NewFromFloat(response.Volume[i]),
			Timestamp: t,
			Source:    f.ID(),
		}
