...
func (s *NotificationService) SendAlertNotification(ctx context.Context, event repository.AlertTriggerEvent, methods []AlertNotificationMethod) error {
	log.Printf("[INFO] [NotificationService] SendAlertNotification: started user=%s alert=%s type=%s", event.UserID, event.AlertID, event.AlertType)
	...
	if s.isQuietHours(prefs) {
		log.Printf("[WARN] [NotificationService] SendAlertNotification: SKIPPED quiet hours active user=%s", event.UserID)
		return nil
	}
...
func (s *NotificationService) SendPushNotification(ctx context.Context, event repository.AlertTriggerEvent) error {
	log.Printf("[INFO] [NotificationService] SendPushNotification: started user=%s alert=%s", event.UserID, event.AlertID)
...
func (s *NotificationService) SendWebhookNotification(ctx context.Context, event repository.AlertTriggerEvent, webhookURL string) error {
	log.Printf("[INFO] [NotificationService] SendWebhookNotification: started user=%s alert=%s url=%s", event.UserID, event.AlertID, webhookURL)
	...
	log.Printf("[ERROR] [NotificationService] SendWebhookNotification: ERROR all retries exhausted user=%s: %v", event.UserID, lastErr)
