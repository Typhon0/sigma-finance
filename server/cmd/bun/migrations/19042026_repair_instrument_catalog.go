...
func init() {
	Migrations.MustRegister(
		func(ctx context.Context, db *bun.DB) error {
			fmt.Print(" [up migration] Repair instrument catalog ")
			_, err := db.ExecContext(ctx, `
				...
				DO $$
				BEGIN
					ALTER TABLE sigma_finance.assets ADD COLUMN IF NOT EXISTS instrument_id UUID;
					ALTER TABLE sigma_finance.portfolio_asset ADD COLUMN IF NOT EXISTS instrument_id UUID;
					IF NOT EXISTS (
						SELECT 1
						FROM pg_constraint
						WHERE conname = 'assets_instrument_id_fkey'
						  AND conrelid = 'sigma_finance.assets'::regclass
					) THEN
					...
					IF NOT EXISTS (
						SELECT 1
						FROM pg_constraint
						WHERE conname = 'portfolio_asset_instrument_id_fkey'
						  AND conrelid = 'sigma_finance.portfolio_asset'::regclass
					) THEN
				...
				EXCEPTION
					WHEN insufficient_privilege THEN
						RAISE NOTICE 'Skipping asset linkage migration because the current role does not own the assets tables';
				END $$;
			`)
			if err != nil {
