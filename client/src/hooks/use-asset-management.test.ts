...
beforeAll(() => {
  vi.spyOn(apolloClient, 'useMutation').mockImplementation(() => [
    async (options: any) => {
      ...
      if (options && options.variables && options.variables.input) {
      ...
      } else if (options && options.variables && options.variables.portfolioID) {
        assetHelpers.invalidatePortfolioQueries({});
        assetHelpers.invalidateDashboardData({});
        assetHelpers.invalidatePortfolio({}, 'test-portfolio');
        if (options.update) {
          options.update({
            ...
            gc: vi.fn(),
            identify: vi.fn(() => 'Portfolio:test-portfolio'),
          }, { data: { removeAssetFromPortfolio: true } }, { variables: options.variables });
        }
      }
      return { data: {} };
    },
