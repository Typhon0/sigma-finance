...
import { useMonitoring, useDashboardStateTracking, useActionTracking } from '@/components/monitoring/monitoring-provider';
...
export function useDashboardMonitoring() {
  ...
  const trackDashboardStateChange = useCallback((newState: Partial<DashboardState>) => {
  ...
  const trackPortfolioSelection = useCallback((portfolioId: string, method: 'click' | 'navigation' = 'click') => {
  ...
  const trackAssetSelection = useCallback((assetId: string, portfolioId: string, method: 'click' | 'navigation' = 'click') => {
    interactionStartTime.current = Date.now();
  ...
  const trackReturnToOverview = useCallback((method: 'breadcrumb' | 'back-button' | 'navigation' = 'breadcrumb') => {
    interactionStartTime.current = Date.now();
  ...
  const trackInteraction = useCallback((action: string, component: string, additionalData?: Record<string, any>) => {
  ...
  const trackChartInteraction = useCallback((chartType: string, action: string, additionalData?: Record<string, any>) => {
  ...
  const trackFormInteraction = useCallback((formName: string, action: string, success?: boolean, additionalData?: Record<string, any>) => {
  ...
  const trackSearchFilter = useCallback((type: 'search' | 'filter', query: string, resultsCount?: number, component?: string) => {
  ...
  const trackError = useCallback((error: Error, component: string, context?: Record<string, any>) => {
  ...
  const trackPerformanceBottleneck = useCallback((bottleneckType: string, duration: number, component: string, details?: Record<string, any>) => {
    monitoring.recordDashboardEvent({
...
export function useComponentMonitoring(componentName: string) {
  ...
  const trackComponentError = useCallback((error: Error, context?: Record<string, any>) => {
  ...
  const trackComponentInteraction = useCallback((action: string, additionalData?: Record<string, any>) => {
...
export function useChartMonitoring(chartType: string) {
  const { trackChartInteraction, trackError, trackPerformanceBottleneck } = useDashboardMonitoring();
  const chartRenderTime = useRef<number>(0);
  const dataPoints = useRef<number>(0);

  const trackChartRender = useCallback((renderTime: number, dataPointCount: number) => {
  ...
  const trackChartError = useCallback((error: Error, context?: Record<string, any>) => {
  ...
  const trackChartDataUpdate = useCallback((updateTime: number, newDataPointCount: number) => {
