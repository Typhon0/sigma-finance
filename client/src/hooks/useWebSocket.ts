...
export function usePriceUpdates(assetIds?: string[]) {
  ...
  const getPriceForAsset = useCallback((assetId: string) => {
...
export function usePortfolioUpdates(portfolioIds?: string[]) {
  ...
  useEffect(() => {
    const unsubscribe = wsManager.subscribe('PORTFOLIO_UPDATE', (message: WebSocketMessage) => {
      ...
      if (!portfolioIdsRef.current || portfolioIdsRef.current.includes(portfolioUpdate.portfolioId)) {
  ...
  const getPortfolioUpdate = useCallback((portfolioId: string) => {
