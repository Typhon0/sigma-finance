...
export const useAuthentication = () => {
	...
	const login = useCallback(
		async (email: string, password: string) => {
			return handleAuthAction(() => auth.login(email, password), "login");
		},
	...
	const logout = useCallback(async () => {
		return handleAuthAction(() => auth.logout(), "logout");
	}, [auth.logout, handleAuthAction]);

	const resetPassword = useCallback(
		async (email: string) => {
			return handleAuthAction(() => auth.resetPassword(email), "resetPassword");
		},
	...
	const verifyEmail = useCallback(
		async (token: string) => {
			return handleAuthAction(() => auth.verifyEmail(token), "verifyEmail");
		},
	...
	const refreshToken = useCallback(async () => {
		return handleAuthAction(() => auth.refreshToken(), "refreshToken");
