...
function dashboardReducer(state: DashboardState, action: DashboardAction): DashboardState {
  ...
  if (action.type !== 'START_TRANSITION' && action.type !== 'END_TRANSITION') {
    if (now - state.lastUpdate < 16) { // 60fps throttling
      return state;
    }
  }
    ...
    switch (action.type) {
      ...
      case 'VIEW_ASSET': {
        ...
        return {
          ...
          selectedPortfolio: portfolio,
          selectedAsset: asset,
          breadcrumbPath: createAssetBreadcrumb(asset, portfolio, () => {}, () => {}), // Will be updated with actual callbacks
          lastUpdate: now,
        };
      }
      ...
      case 'UPDATE_PORTFOLIO': {
        ...
        return {
          ...state,
          selectedPortfolio: state.selectedPortfolio?.id === updatedPortfolio.id 
            ? updatedPortfolio 
      ...
      case 'UPDATE_ASSET': {
        ...
        return {
          ...state,
          selectedAsset: state.selectedAsset?.id === updatedAsset.id 
            ? updatedAsset 
            : state.selectedAsset,
          lastUpdate: now,
        };
...
export function useOptimizedDashboardState() {
  ...
  const actions = useMemo(() => ({
    ...
    viewAsset: (asset: Asset, portfolio: Portfolio) => {
      ...
      setTimeout(() => {
        throttledDispatch({ type: 'VIEW_ASSET', payload: { asset, portfolio } });
        throttledDispatch({ type: 'END_TRANSITION' });
      }, 0);
    },
...
export function useContextSwitchingOptimizer() {
  ...
  const recordContextSwitch = useCallback((fromContext: string, toContext: string) => {
    switchCount.current++;
    const now = Date.now();
    ...
    if (switchTime > 100) {
      console.warn(`Slow context switch from ${fromContext} to ${toContext}: ${switchTime}ms`);
    }
  }, []);

  const getContextSwitchStats = useCallback(() => {
    const times = switchTimes.current;
    const avgSwitchTime = times.length > 0 
      ? times.reduce((sum, time) => sum + time, 0) / times.length 
      : 0;
