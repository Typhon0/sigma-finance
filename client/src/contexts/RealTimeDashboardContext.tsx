...
import { usePriceUpdates, usePortfolioUpdates, useAlertNotifications, useWebSocket } from '@/hooks/useWebSocket';

export interface DashboardState {
  // Price data
  assetPrices: Map<string, {
    price: number;
    change: number;
...
type DashboardAction = 
  ...
  | { type: 'SET_CONNECTION_STATE'; payload: { connected: boolean; connecting: boolean; error: string | null } }
...
function dashboardReducer(state: DashboardState, action: DashboardAction): DashboardState {
  switch (action.type) {
    ...
    case 'UPDATE_PORTFOLIO':
      return {
        ...state,
        portfolioValues: new Map(state.portfolioValues.set(action.payload.portfolioId, action.payload.data))
      };
    ...
    case 'ADD_ALERT':
      return {
        ...
        unreadAlertCount: action.payload.acknowledged ? state.unreadAlertCount : state.unreadAlertCount + 1
      };
