...
export function PerformanceDashboard({ className }: PerformanceDashboardProps) {
  ...
  const { metrics, loading: metricsLoading, refetch: refetchMetrics } = usePerformanceMetrics(timeRange);
  ...
  const { realtimeMetrics } = useRealTimePerformanceMonitoring();
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {metrics?.dataLoadTimes?.average ? formatDuration(metrics.dataLoadTimes.average * 1000) : '--'}
            </div>
            <p className="text-xs text-muted-foreground">
              P95: {metrics?.dataLoadTimes?.p95 ? formatDuration(metrics.dataLoadTimes.p95 * 1000) : '--'}
            </p>
          </CardContent>
        </Card>
      ...
      {health && health.recentErrors > 5 && (
        <Alert>
          ...
          <AlertDescription>
            System is experiencing {health.recentErrors} errors in the last 5 minutes. 
            ...
            <CardContent>
              <div className="space-y-4">
                {metrics?.dashboardStateTransitions?.breakdown && 
                  Object.entries(metrics.dashboardStateTransitions.breakdown).map(([component, value]) => (
                  ...
                  {engagement?.errorsByComponent && 
                    Object.entries(engagement.errorsByComponent).map(([component, count]) => (
                      <div key={component} className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4">
                          <Progress 
                            value={Math.min((count / Math.max(...Object.values(engagement.errorsByComponent))) * 100, 100)} 
                            className="w-24" 
                          />
        ...
        <TabsContent value="market-data" className="space-y-4">
          <div className="grid gap-4 lg:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {metrics?.marketDataUpdates?.average ? formatDuration(metrics.marketDataUpdates.average * 1000) : '--'}
                </div>
                <p className="text-xs text-muted-foreground">
