...
export function PerformanceDashboard({ className }: PerformanceDashboardProps) {
	...
	const { engagement, loading: engagementLoading } =
		useUserEngagementMetrics(timeRange);
	const { _realtimeMetrics } = useRealTimePerformanceMonitoring();
	...
	const deviceChartData = useMemo(() => {
		...
		return Object.entries(engagement.deviceBreakdown).map(
			([device, count]) => ({
				name: device,
				value: count,
				color:
		...
		<div className={`space-y-6 ${className}`}>
			...
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
				...
				<Card>
					...
					<CardContent>
						<div className="text-2xl font-bold">
							{metrics?.errorRates?.lastHour
								? `${metrics.errorRates.lastHour.toFixed(1)}%`
								: "0%"}
						</div>
						<p className="text-xs text-muted-foreground">
			...
			{health && health.recentErrors > 5 && (
				<Alert>
					...
					<AlertDescription>
						System is experiencing {health.recentErrors} errors in the last 5
						minutes. Performance may be degraded.
					</AlertDescription>
				</Alert>
			)}
			...
			<Tabs defaultValue="performance" className="space-y-4">
					...
					<div className="grid gap-4 lg:grid-cols-2">
									...
									<div className="flex justify-between items-center">
										...
										<span className="font-medium">
											{metrics?.dashboardStateTransitions?.average
												? formatDuration(
														metrics.dashboardStateTransitions.average * 1000,
													)
												: "--"}
										</span>
									</div>
									<div className="flex justify-between items-center">
										...
										<span className="font-medium">
											{metrics?.dashboardStateTransitions?.p95
												? formatDuration(
														metrics.dashboardStateTransitions.p95 * 1000,
													)
												: "--"}
										</span>
									</div>
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Dashboard data loading times and success rates
								</CardDescription>
							</CardHeader>
							<CardContent>
								<div className="grid gap-4 mb-4">
				...
				<TabsContent value="engagement" className="space-y-4">
					<div className="grid gap-4 lg:grid-cols-2">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Most used dashboard features and user engagement
								</CardDescription>
							</CardHeader>
							<CardContent>
								<div className="space-y-4">
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Common user navigation flows and conversion rates
							</CardDescription>
						</CardHeader>
						<CardContent>
							<div className="space-y-4">
									...
									>
										<div className="flex-1">
											...
											<div className="text-sm text-muted-foreground">
												{pattern.count} users • Avg:{" "}
												{formatDuration(pattern.avgDuration * 1000)}
											</div>
										</div>
										<div className="flex items-center gap-2">
											<Badge
												variant={
													pattern.conversion > 0.8 ? "default" : "secondary"
												}
											>
					...
					<div className="grid gap-4 lg:grid-cols-2">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Error occurrence patterns over time
								</CardDescription>
							</CardHeader>
							<CardContent>
							...
							<CardContent>
								<div className="space-y-4">
									{engagement?.errorsByComponent &&
										Object.entries(engagement.errorsByComponent).map(
											([component, count]) => (
												<div
													key={component}
													className="flex items-center justify-between"
				...
				<TabsContent value="market-data" className="space-y-4">
					...
					<Card>
						...
						<CardContent>
							<div className="space-y-4">
								{["Alpha Vantage", "Yahoo Finance", "CoinGecko"].map(
									(source) => (
										<div
											key={source}
											className="flex items-center justify-between p-4 border rounded-lg"
