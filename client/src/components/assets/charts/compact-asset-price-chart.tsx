...
import { AssetPriceChart, type AssetPriceChartProps, type PricePoint, type AssetPrice } from './asset-price-chart';
import { ResponsiveChartContainer, DashboardChartContainer } from './responsive-chart-container';
 ...
 */
export interface CompactChartProps extends Omit<AssetPriceChartProps, 'height' | 'compact' | 'showControls'> {
...
export function AssetPriceSparkline({
  ...
  const strokeColor = isPositive ? '#22c55e' : currentPrice?.change < 0 ? '#ef4444' : '#6b7280';
  ...
  return (
    ...
    >
      ...
      {currentPrice && (
        <circle
          cx={width}
          cy={height - ((data[data.length - 1].close - Math.min(...data.map(d => d.close))) / 
              (Math.max(...data.map(d => d.close)) - Math.min(...data.map(d => d.close)) || 1)) * height}
          r="2"
          fill={strokeColor}
        />
      )}
