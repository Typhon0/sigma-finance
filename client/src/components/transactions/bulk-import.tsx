...
export function BulkImport({
  ...
  const validateTransaction = (transaction: ParsedTransaction) => {
    ...
    if (!transaction.type) {
      ...
      const validTypes = ['BUY', 'SELL', 'DEPOSIT', 'WITHDRAWAL', 'TRANSFER_IN', 'TRANSFER_OUT', 'DIVIDEND', 'INTEREST', 'FEE', 'ADJUSTMENT'];
        ...
        {csvData && !parsedTransactions.length && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {Object.entries(csvColumnMapping).map(([field, possibleNames]) => (
          ...
          <div className="space-y-4">
            ...
            {invalidTransactionCount > 0 && (
              <Alert>
                ...
                <AlertDescription>
                  {invalidTransactionCount} transaction(s) have validation errors. 
        ...
        <div className="flex gap-3 pt-4">
          {parsedTransactions.length > 0 && selectedPortfolio && (
            <Button
              ...
              className="flex-1"
            >
              {isLoading ? 'Processing...' : validationOnly ? 'Validate Transactions' : 'Import Transactions'}
            </Button>
          )}
