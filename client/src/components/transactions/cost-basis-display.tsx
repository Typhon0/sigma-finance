...
import { TrendingUp, TrendingDown, Calculator, Info, Eye, EyeOff } from 'lucide-react';
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
                ...
                <div>
                  ...
                  <div className={`flex items-center gap-2 ${gainLossColor}`}>
                    ...
                    <span className="text-xl font-bold">
                      {isGain ? '+' : ''}{formatCurrency(unrealizedGainLoss)}
                    </span>
                  </div>
                  <p className={`text-sm ${gainLossColor}`}>
                    {formatPercentage(unrealizedGainLossPercent)}
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className={`flex items-center gap-2 ${realizedGainLoss >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {realizedGainLoss >= 0 ? (
        ...
        <Alert>
          ...
          <AlertDescription>
            Cost basis is calculated using the FIFO (First In, First Out) method. 
            This means the oldest shares are considered sold first when calculating realized gains/losses.
          </AlertDescription>
        </Alert>
              ...
              <div className="border rounded-lg p-4 space-y-3">
                ...
                <div className="space-y-2">
                  {transactions.map((transaction, index) => {
                    const transactionValue = transaction.quantity * transaction.pricePerUnit;
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-medium">
                              {transaction.quantity.toLocaleString()} shares @ {formatCurrency(transaction.pricePerUnit)}
                            </p>
                            <p className="text-xs text-muted-foreground">
                        ...
                        <div className="text-right">
                          <p className={`font-medium ${isBuy ? 'text-red-600' : 'text-green-600'}`}>
                            {isBuy ? '-' : '+'}{formatCurrency(transactionValue)}
                          </p>
                        </div>
                      </div>
                    );
                ...
                <div className="text-sm text-muted-foreground space-y-2">
                  ...
                  <p>
                    ...
                    <span className={gainLossColor}>
                      {' '}{formatCurrency(currentValue)} - {formatCurrency(totalCostBasis)} = {formatCurrency(unrealizedGainLoss)}
                    </span>
                  </p>
                </div>
              </div>
