...
export function DisplayCurrencySelector({
	...
	const [updateDisplayCurrency, { loading }] = useUpdateDisplayCurrencyMutation();
	...
	const handleSave = async () => {
		try {
			...
			if (result.data?.updateUserDisplayCurrency) {
				...
				toast.success(`Display currency updated to ${selectedCurrency}`);
	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>Choose the currency for displaying your portfolio values</CardDescription>
			</CardHeader>
			<CardContent>
				<RadioGroup value={selectedCurrency} onValueChange={setSelectedCurrency} disabled={loading}>
                      ...
                      `}
										...
										<div className="flex-1">
											<div className="flex items-center gap-2">
												<RadioGroupItem value={currency.id} id={currency.id} className="sr-only" />
