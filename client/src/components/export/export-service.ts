import { ExportRequest, ExportResult, ExportType, ExportFormat } from './types';
...
export class ExportService {
  ...
  private static generateFilename(type: ExportType, format: ExportFormat, portfolioName?: string): string {
  ...
  static async exportPortfolioData(
    ...
    try {
      ...
      const mockData = await this.fetchPortfolioData(portfolioId);
      ...
      switch (format) {
        case 'csv':
          ...
          filename = this.generateFilename('portfolio-data', 'csv', mockData.portfolio.name);
          break;
        
        case 'pdf':
          blob = await PDFGenerator.generatePortfolioReport(mockData);
          filename = this.generateFilename('portfolio-data', 'pdf', mockData.portfolio.name);
          break;
        
        case 'json':
          const jsonContent = JSON.stringify(mockData, null, 2);
          blob = new Blob([jsonContent], { type: 'application/json' });
          filename = this.generateFilename('portfolio-data', 'json', mockData.portfolio.name);
          break;
  ...
  static async exportTransactionHistory(
    ...
    try {
      const mockData = await this.fetchTransactionHistory(portfolioId, dateRange);
      ...
      switch (format) {
        case 'csv':
          ...
          blob = new Blob([csvContent], { type: 'text/csv' });
          filename = this.generateFilename('transaction-history', 'csv');
          break;
        
        case 'json':
          const jsonContent = JSON.stringify(mockData, null, 2);
          blob = new Blob([jsonContent], { type: 'application/json' });
          filename = this.generateFilename('transaction-history', 'json');
          break;
  ...
  static async exportTaxReport(
    ...
    try {
      const mockData = await this.fetchTaxReportData(userId, taxYear);
      ...
      switch (format) {
        case 'csv':
          ...
          blob = new Blob([csvContent], { type: 'text/csv' });
          filename = this.generateFilename('tax-report', 'csv');
          break;
        
        case 'pdf':
          blob = await PDFGenerator.generateTaxReport(mockData);
          filename = this.generateFilename('tax-report', 'pdf');
          break;
        
        case 'json':
          const jsonContent = JSON.stringify(mockData, null, 2);
          blob = new Blob([jsonContent], { type: 'application/json' });
          filename = this.generateFilename('tax-report', 'json');
          break;
  ...
  static async exportAuditTrail(
    ...
    try {
      const mockData = await this.fetchAuditTrailData(userId, dateRange);
      ...
      switch (format) {
        case 'csv':
          ...
          blob = new Blob([csvContent], { type: 'text/csv' });
          filename = this.generateFilename('audit-trail', 'csv');
          break;
        
        case 'json':
          const jsonContent = JSON.stringify(mockData, null, 2);
          blob = new Blob([jsonContent], { type: 'application/json' });
          filename = this.generateFilename('audit-trail', 'json');
          break;
  ...
  static async createDataBackup(userId: string): Promise<ExportResult> {
    try {
      const backupData = await this.fetchCompleteUserData(userId);
      
      const jsonContent = JSON.stringify(backupData, null, 2);
      const blob = new Blob([jsonContent], { type: 'application/json' });
      const filename = this.generateFilename('data-backup', 'json');
  ...
  private static async fetchTaxReportData(userId: string, taxYear: number): Promise<any> {
    // Mock implementation
    return {
      taxYear,
      userId,
