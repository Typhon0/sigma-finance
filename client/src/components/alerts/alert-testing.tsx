...
import type { Alert as AlertType, AlertTestData } from './types';
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <Alert>
        ...
        <AlertDescription>
          Use this testing interface to validate your alert configurations and notification delivery. 
          Tests will simulate alert conditions without affecting your actual alerts.
        </AlertDescription>
      </Alert>
        ...
        <CardContent>
          <form onSubmit={handleSubmit(handleTest)} className="space-y-4">
                    ...
                    {alerts.map((alert) => (
                      <SelectItem key={alert.id} value={alert.id}>
                        <div className="flex items-center gap-2">
                          ...
                          <span>{alert.name || `${getAlertTypeLabel(alert.alertType)} - ${getTargetName(alert)}`}</span>
                        </div>
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            {selectedAlert && (
              <div className="p-4 bg-muted/50 rounded-lg space-y-2">
                ...
                <div className="text-sm text-muted-foreground">
                  Target: {getTargetName(selectedAlert)} | Threshold: {formatThreshold(selectedAlert)}
                </div>
            ...
            {watchedTestType === 'CONDITION' && selectedAlert && (
              <div className="grid gap-4 grid-cols-1 md:grid-cols-2">
                {(selectedAlert.alertType === 'PRICE' || selectedAlert.alertType === 'PORTFOLIO_VALUE') && (
                  <div className="space-y-2">
                ...
                {(selectedAlert.alertType === 'PERCENTAGE_CHANGE' || selectedAlert.alertType === 'ALLOCATION') && (
                  <div className="space-y-2">
