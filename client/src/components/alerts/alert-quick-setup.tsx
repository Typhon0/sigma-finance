...
import type { AlertQuickSetupData, Portfolio, Asset } from './types';
...
export function AlertQuickSetup({
  ...
  const selectedPortfolio = portfolios.find(p => p.id === watchedPortfolioId);
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              {selectedAsset && selectedAsset.currentPrice && (
                <div className="p-3 bg-muted/50 rounded-lg">
                  <p className="text-sm text-muted-foreground">
                    Current price of {selectedAsset.name}: 
                    <span className="font-medium ml-1">${selectedAsset.currentPrice.toFixed(2)}</span>
                  </p>
                </div>
              )}
            </div>
