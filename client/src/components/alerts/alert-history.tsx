...
import type { AlertHistoryData, AlertFilterData } from './types';
...
export function AlertHistory({
  ...
  const filteredHistory = history.filter(item => {
    ...
    const matchesDateRange = (!dateRange.from || item.triggeredAt >= dateRange.from) &&
          ...
          .map(([date, items]) => (
            <div key={date} className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <Badge variant="outline">{items.length} trigger{items.length !== 1 ? 's' : ''}</Badge>
              </div>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="space-y-2">
                              ...
                              {(item.currentValue !== undefined || item.thresholdValue !== undefined) && (
