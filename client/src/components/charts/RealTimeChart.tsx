...
export function RealTimeChart({
	...
	}, [chartType, height, showVolume, responsiveState.isMobile]);
	...
	}, [assetId, chartType, lastPrice, actions]);
			...
			>
				...
				>
					<div className="flex items-center gap-2">
						<CardTitle className={cn(responsiveState.isMobile ? "text-base" : "text-lg")}>
							{symbol}
						</CardTitle>
						{getTrendIcon()}
					</div>
				...
				{currentPrice && (
					...
					>
						<span className={cn("font-bold", responsiveState.isMobile ? "text-xl" : "text-2xl")}>
							{formatPrice(currentPrice.price)}
						</span>
						<span className={cn("text-sm font-medium", getChangeColor(currentPrice.change))}>
				...
				{!isConnected && (
					<div className="absolute inset-0 bg-gray-50/80 flex items-center justify-center">
						<div className="text-center">
							...
							/>
							<p className={cn("text-gray-600", responsiveState.isMobile ? "text-xs" : "text-sm")}>
								Connection lost
							</p>
