...
export function createTooltipConfig(config: ChartTooltipConfig = {}): Partial<EChartsOption> {
  ...
  return {
    tooltip: {
      ...
      formatter: customFormatter || function(params: any) {
 ...
 */
export function createInteractionConfig(config: ChartInteractionConfig = {}): Partial<EChartsOption> {
  const {
    enableZoom = true,
    enablePan = true,
    enableBrush = false,
 ...
 */
export function createLegendConfig(position: 'top' | 'bottom' | 'left' | 'right' = 'top'): Partial<EChartsOption> {
  const baseConfig = {
    show: true,
    textStyle: {
 ...
 */
export function createResponsiveConfig(breakpoint: 'mobile' | 'tablet' | 'desktop' = 'desktop'): Partial<EChartsOption> {
  switch (breakpoint) {
    case 'mobile':
      return {
        grid: {
 ...
 */
export function createAnimationConfig(enabled: boolean = true, duration: number = 1000): Partial<EChartsOption> {
  if (!enabled) {
    return {
      animation: false,
    };
