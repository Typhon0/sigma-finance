...
import { CompactPerformanceChart, CompactAllocationChart, MiniPerformanceSparkline, MiniAllocationDonut } from '../CompactCharts';
...
const mockAllocationData = [
  { name: 'Stocks', value: 45000, percentage: 45, assetType: 'STOCK', color: '#3b82f6' },
  { name: 'Crypto', value: 25000, percentage: 25, assetType: 'CRYPTO', color: '#f59e0b' },
  { name: 'Bonds', value: 20000, percentage: 20, assetType: 'BOND', color: '#10b981' },
  { name: 'Cash', value: 10000, percentage: 10, assetType: 'BANK_ACCOUNT', color: '#6b7280' },
];
...
describe('Inline Dashboard Charts', () => {
    ...
    it('should handle asset type colors correctly', () => {
      ...
      expect(chartOption.series[0].data).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ name: 'Stocks', itemStyle: { color: '#3b82f6' } }),
          expect.objectContaining({ name: 'Crypto', itemStyle: { color: '#f59e0b' } }),
          expect.objectContaining({ name: 'Bonds', itemStyle: { color: '#10b981' } }),
          expect.objectContaining({ name: 'Cash', itemStyle: { color: '#6b7280' } }),
        ])
      );
    });
  ...
  describe('Chart Accessibility', () => {
    ...
    it('should provide alternative text for screen readers', () => {
      ...
      expect(screen.getByText('Asset allocation: 45% Stocks, 25% Crypto, 20% Bonds, 10% Cash')).toBeInTheDocument();
    });
  });
