...
const legacyAssetTypeMap: Partial<Record<AssetType, Exclude<AssetType, "cryptos" | "moneymarkets">>> =
  {
...
export function SyncToggleCard({
  ...
  const config = normalizedAssetType
    ? assetTypeConfig[normalizedAssetType]
    : undefined;
      ...
      <CardContent className="pt-0">
        ...
        {isEnabled && syncStatus !== "syncing" && !isBusy && (
          <button
            type="button"
            onClick={onSync}
            className="mt-3 w-full flex items-center justify-center gap-2 rounded-md bg-primary px-3 py-1.5 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors disabled:pointer-events-none disabled:opacity-50"
          >
