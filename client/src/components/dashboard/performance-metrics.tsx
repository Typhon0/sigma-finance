...
import { ArrowDown, ArrowUp, TrendingUp, TrendingDown, DollarSign, Percent, Target, AlertTriangle } from 'lucide-react';
...
const getChangeIndicator = (change?: number, changePercent?: number) => {
  ...
  const displayValue = changePercent !== undefined ? formatPercentage(changePercent) : formatCurrency(change!);
  const isPositive = (changePercent !== undefined ? changePercent : change!) > 0;
  
  return (
    <div className={cn("flex items-center gap-1 text-sm", getPerformanceColorClass(changePercent !== undefined ? changePercent : change!))}>
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="text-right space-y-1">
            ...
            {hasTarget && (
              <div className="flex items-center gap-1 text-xs">
                ...
                <span className={cn(
                  ...
                  {targetDifference >= 0 ? "+" : ""}{formatMetricValue(targetDifference, metric.format)}
                </span>
              </div>
            )}
            ...
            {showBenchmark && hasBenchmark && (
              <div className="flex items-center gap-1 text-xs">
                <Badge variant={benchmarkDifference >= 0 ? "default" : "destructive"} className="text-xs px-1">
                  vs Benchmark: {benchmarkDifference >= 0 ? "+" : ""}{formatMetricValue(benchmarkDifference, metric.format)}
                </Badge>
              </div>
            )}
          </div>
