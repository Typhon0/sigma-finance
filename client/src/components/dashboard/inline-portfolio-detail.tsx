...
import type { Portfolio, Position, Transaction } from "@/gql/graphql";
...
export function InlinePortfolioDetail({
	...
	const totalValue =
		realTimePortfolioData?.totalValue ||
		assets.reduce((sum, position) => {
		...
		>
			...
			>
				...
				>
					...
					<MetricCard title="Assets" value={assets.length.toString()} />
				</div>
			</div>
			...
			<Tabs defaultValue="overview" className="w-full">
				...
				<TabsContent
					value="overview"
					className={cn(responsiveState.isMobile ? "space-y-4" : "space-y-6")}
				>
					{/* Real-Time Charts */}
					<div
						className={cn(
