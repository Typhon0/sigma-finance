...
import { Bell, Plus, Edit, Trash2, TrendingUp, TrendingDown, Target, AlertTriangle } from 'lucide-react';
...
const alertTypes = [
  { value: 'price', label: 'Asset Price', description: 'Alert when asset price reaches threshold' },
  { value: 'percentage', label: 'Price Change %', description: 'Alert on percentage price change' },
  { value: 'portfolio_value', label: 'Portfolio Value', description: 'Alert when portfolio value changes' },
  { value: 'allocation', label: 'Asset Allocation', description: 'Alert when allocation exceeds limits' },
  { value: 'performance', label: 'Performance', description: 'Alert on performance metrics' },
];
...
const conditions = [
  ...
  { value: 'increase_by', label: 'Increases By', description: 'Trigger when value increases by amount' },
  { value: 'decrease_by', label: 'Decreases By', description: 'Trigger when value decreases by amount' },
];

const frequencies = [
  { value: 'immediate', label: 'Immediate', description: 'Alert immediately when condition is met' },
...
const AlertForm: React.FC<{
  ...
  const needsPortfolioTarget = ['portfolio_value', 'allocation', 'performance'].includes(formData.type);
          ...
          <div className="flex items-start gap-3 flex-1">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <h4 className={cn("font-semibold truncate", compact && "text-sm")}>
                  {alert.name}
                </h4>
              ...
              <p className={cn("text-sm text-muted-foreground mb-2", compact && "text-xs")}>
                {typeInfo?.label} {conditionInfo?.label.toLowerCase()} {formatThreshold(alert.threshold, alert.type)}
              </p>
...
export const PerformanceAlerts: React.FC<PerformanceAlertsProps> = ({
  ...
  const alertStats = useMemo(() => {
    ...
    const triggered = alerts.filter(a => 
      a.lastTriggered && Date.now() - a.lastTriggered.getTime() < 24 * 60 * 60 * 1000
    ).length;
        ...
        {alerts.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-muted-foreground mb-4">
              Create alerts to monitor your portfolio performance and get notified of important changes.
            </p>
