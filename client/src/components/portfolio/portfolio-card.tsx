...
import { MoreHorizontal, Edit, Copy, Download, Trash, GripVertical } from 'lucide-react'
...
export type PortfolioAction = 'edit' | 'delete' | 'duplicate' | 'export' | 'view'
...
interface PortfolioCardProps {
  portfolio: Portfolio & { analytics?: PortfolioAnalytics }
  viewMode: ViewMode
  isSelected: boolean
...
export function PortfolioCard({
  ...
  const analytics = portfolio.analytics || calculatePortfolioAnalytics(portfolio.assets);

  // Find assets belonging to this portfolio
  const portfolioAssets = assets
    ? assets.filter((a: any) => a.portfolioID === portfolio.id)
    : [];
        ...
        <div className={cn(
          ...
          <div className={cn(
            ...
            <div className="text-right">
              <Badge variant={getPerformanceVariant(analytics.totalGainLossPercent)}>
          ...
          {portfolioAssets.length > 0 && (
            <div className="mt-2 text-xs text-muted-foreground">
              {portfolioAssets.slice(0, 3).map((a: any) => (
                <span key={a.id} className="mr-2">{a.name}</span>
              ))}
              {portfolioAssets.length > 3 && <span>+{portfolioAssets.length - 3} more</span>}
            </div>
          )}
...
// Utility functions
function calculatePortfolioAnalytics(assets: PortfolioAsset[]): PortfolioAnalytics {
  ...
  assets.forEach(asset => {
    const currentValue = asset.asset.currentValue * asset.quantity * (asset.ownershipPct / 100)
    const cost = asset.averagePurchasePrice * asset.quantity * (asset.ownershipPct / 100)
...
function getPerformanceVariant(percent: number): 'default' | 'secondary' | 'destructive' {
