...
export class AuthErrorHandler {
	...
	static showErrors(errors: AuthError[]): void {
		...
		const message =
			ERROR_MESSAGES[firstError.code as AuthErrorCode] || firstError.message;
		toast.error(
			`${message} (${errors.length - 1} more error${errors.length > 2 ? "s" : ""})`,
		);
	}
	...
	static getFieldError(errors: AuthError[], field: string): string | undefined {
		...
		return (
			ERROR_MESSAGES[fieldError.code as AuthErrorCode] || fieldError.message
		);
	}
		...
		errors.forEach((error) => {
			if (error.field) {
				fieldErrors[error.field] =
					ERROR_MESSAGES[error.code as AuthErrorCode] || error.message;
			}
		});
