...
const ERROR_MESSAGES: Record<AuthErrorCode, string> = {
  ...
  [AUTH_ERROR_CODES.ACCOUNT_LOCKED]: 'Your account has been temporarily locked due to too many failed login attempts. Please try again later.',
  ...
  [AUTH_ERROR_CODES.RATE_LIMIT_EXCEEDED]: 'Too many requests. Please wait a moment before trying again.',
  [AUTH_ERROR_CODES.WEAK_PASSWORD]: 'Password must be at least 8 characters long and contain a mix of letters, numbers, and symbols.',
...
export class AuthErrorHandler {
  ...
  static showError(error: AuthError): void {
    const message = ERROR_MESSAGES[error.code as AuthErrorCode] || error.message || 'An unexpected error occurred.';
    toast.error(message);
  }
