...
import type { CandlestickData, LineData, AreaData, HistogramData, UTCTimestamp } from './lightweight-charts';
...
export class LightweightChartsFormatter {
  ...
  static toCandlestickData(data: RawPriceData[]): CandlestickData[] {
    return data
      ...
      .map(item => ({
        time: this.toChartTime(item.timestamp),
        open: item.open,
        high: item.high,
        low: item.low,
        close: item.close,
  ...
  static toLineData(data: RawPerformanceData[]): LineData[] {
    return data
      ...
      .map(item => ({
        time: this.toChartTime(item.date),
        value: item.value,
      }))
      .sort((a, b) => (a.time as number) - (b.time as number));
  }
...
export class EChartsFormatter {
  ...
  static formatPerformanceData(data: RawPerformanceData[]): {
    ...
    return {
      dates: sortedData.map(item => this.formatDateForAxis(item.date)),
      values: sortedData.map(item => item.value),
      formattedData: sortedData.map(item => ({
        date: this.formatDateForAxis(item.date),
        value: item.value,
        label: this.formatCurrency(item.value),
      })),
    };
  }
  ...
  static formatAllocationData(data: RawAllocationData[]): Array<{
    ...
    return data
      .filter(item => item && item.name && typeof item.value === 'number' && item.value > 0)
      .map(item => ({
        name: item.name,
        value: item.value,
...
export class DataValidator {
   ...
   * Validate price data
   */
  static validatePriceData(data: any[]): { valid: boolean; errors: string[]; cleanData: RawPriceData[] } {
    const errors: string[] = [];
   ...
   */
  static validatePerformanceData(data: any[]): { valid: boolean; errors: string[]; cleanData: RawPerformanceData[] } {
    const errors: string[] = [];
   ...
   */
  static validateAllocationData(data: any[]): { valid: boolean; errors: string[]; cleanData: RawAllocationData[] } {
    const errors: string[] = [];
