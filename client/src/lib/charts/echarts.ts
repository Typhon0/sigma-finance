...
import { getChartColors, getChartThemeColors, getAssetTypeColor } from '../chart-colors';
 ...
 */
export function getEChartsTheme(isDark: boolean = false): EChartsTheme {
 ...
 */
export function getDefaultEChartsConfig(theme: EChartsTheme): Partial<EChartsOption> {
  return {
...
export function createAllocationPieChart(
  ...
  const seriesData = data.map((item, index) => ({
    ...
    value: item.value,
    itemStyle: {
      color: item.assetType ? getAssetTypeColor(item.assetType) : chartColors[index % chartColors.length],
    },
  }));
  ...
  return {
    ...
    tooltip: {
      ...
      trigger: 'item',
      formatter: options.showPercentage !== false 
        ? '{a} <br/>{b}: {c} ({d}%)'
        : '{a} <br/>{b}: {c}',
    },
    legend: {
...
export class EChartsPerformanceManager {
   ...
   */
  static sampleData<T>(data: T[], maxPoints: number = this.MAX_DATA_POINTS): T[] {
