...
import { type ISeriesApi, type CandlestickData, type LineData, type UTCTimestamp } from 'lightweight-charts';
...
export function useRealTimePrices(
  ...
  useEffect(() => {
    assetIds.forEach(assetId => subscribe(assetId));
    
    return () => {
      assetIds.forEach(assetId => unsubscribe(assetId));
    };
