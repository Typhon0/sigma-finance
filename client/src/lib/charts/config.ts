...
export class ChartValidator {
   ...
   */
  static validateConfig(config: ChartConfig): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
    ...
    if (config.library === 'lightweight' && !['candlestick', 'line', 'area', 'histogram'].includes(config.type)) {
      ...
      case 'candlestick':
        data.forEach((item, index) => {
          if (!item.time || !item.open || !item.high || !item.low || !item.close) {
            errors.push(`Candlestick data item ${index} is missing required fields (time, open, high, low, close)`);
          }
