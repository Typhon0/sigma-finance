...
export class TokenManager {
  ...
  static isTokenExpired(expiresAt?: string): boolean {
    if (!expiresAt) {
      const stored = this.getTokenExpiry();
  ...
  static hasValidToken(): boolean {
    ...
    return !!(token && expiry && !this.isTokenExpired(expiry));
  }
}
...
export class AuthRedirect {
  ...
  static getLoginUrl(returnTo?: string): string {
    const url = new URL(this.LOGIN_PATH, window.location.origin);
    if (returnTo) {
...
export class SessionManager {
  ...
  static shouldShowSessionWarning(): boolean {
    ...
    return timeUntilExpiry > 0 && timeUntilExpiry <= this.SESSION_WARNING_TIME;
  }
