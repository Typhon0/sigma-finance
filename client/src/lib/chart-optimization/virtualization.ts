...
export class ChartVirtualizer {
	 ...
	 */
	private calculateDataRange(viewport: ViewportInfo): {
		startIndex: number;
		endIndex: number;
	} {
		...
		if (totalTimeRange <= 0 || this.state.totalDataPoints <= 0) {
			return {
				startIndex: 0,
				endIndex: Math.min(100, this.state.totalDataPoints),
			};
		}
	...
	private cleanupOldChunks(): void {
		...
		if (this.state.chunks.size > maxChunks) {
			const sortedChunks = Array.from(this.state.chunks.entries()).sort(
